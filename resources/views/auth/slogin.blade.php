...
{{-- SUCCESS MESSAGE --}}
        ...
        @endif
            <!-- Laravel Login Form -->
            <form class="login100-form validate-form" method="POST" action="{{ route('auth.slogin.submit') }}">
                @csrf

                <span class="login100-form-logo">
                    <img src="{{ asset('storage/' . $company->logo) }}"
                        alt="{{ $company->name ?? 'Company Logo' }}"
                        class="login-logo-img" style="width: 100%; height: 100%; object-fit: contain; border-radius: 50%;">
                </span>

...
class PurchaseOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        $stats = [
            'total' => PurchaseOrder::whereBelongsTo(Auth::user())->where('company_id', $companyId)->count(),
            'pending' => PurchaseOrder::whereBelongsTo(Auth::user())->where('company_id', $companyId)
                ->whereIn('status', ['draft', 'pending_approval'])->count(),
            'approved' => PurchaseOrder::whereBelongsTo(Auth::user())->where('company_id', $companyId)
                ->where('status', 'approved')->count(),
            'ordered' => PurchaseOrder::whereBelongsTo(Auth::user())->where('company_id', $companyId)
                ->where('status', 'ordered')->count(),
            'completed' => PurchaseOrder::whereBelongsTo(Auth::user())->where('company_id', $companyId)
                ->where('status', 'completed')->count(),
            'total_value' => PurchaseOrder::whereBelongsTo(Auth::user())->where('company_id', $companyId)->sum('total_amount'),
        ];
        
        return view('company.purchase-order.index', compact('purchaseOrders', 'company', 'stats'));
    }
    ...
    {
        return view('company.purchase-order.show', compact('purchaseOrder'));
    }
    ...
    public function download(PurchaseOrder $purchaseOrder)
    {
        ...
        $pdf = PDF::loadView('company.purchase-order.pdf', compact('purchaseOrder', 'company'))
